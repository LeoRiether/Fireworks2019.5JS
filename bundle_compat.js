// It's almost the 20s and I still don't know how to configure babel correctly
// So I just got this from babeljs.io/repl
...
var App = function () {
  ...
  var lerpers = []; // Particles that lerp towards a point

  var fusers = []; // Particles that roam free until they explode

  var roamers = []; // Particles that just move until they dissapear

  function explode(pos, color, n) {
    for (var i = 0; i < n; i++) {
      var abs = Math.random() * 500;
      var theta = Math.random() * Tau;
      var vel = [abs * Math.cos(theta), abs * Math.sin(theta) - 20];
      roamers.push(new Roamer(pos.slice(), vel, Object.create(color)));
    }
  }
  ...
  var lerp = function lerp(from, to, perc) {
    return [perc * (to[0] - from[0]) + from[0], perc * (to[1] - from[1]) + from[1]];
  };
  ...
  function roam(pos, vel, dt) {
    ...
    // I think
    pos[0] += vel[0] * dt; // Velocity-Verlet integration for the y-axis
  ...
  function update(dt) {
    var dts = dt / 1000.0; // roamers.map (dimBy(80*dts))
    //        .filter (p.color.l > 0)
    //        .map (roam)
    //        .map (scaleVelocity(0.95))
    // Roamers

    for (var i = roamers.length - 1; i >= 0; i--) {
      var p = roamers[i];
      p.color.l -= 80 * dts;
    ...
    for (var _i = lerpers.length - 1; _i >= 0; _i--) {
      var _p = lerpers[_i];
      _p.t += dt; // Could remove these kinds of ifs by using maps and filters, but nah

      if (_p.t >= 1000) {
        explode(_p.pos, _p.color, 1);
        removeAt(lerpers, _i);
      } else {
        _p.pos = qerp(_p.from, _p.to, _p.t / 1000.0);
      }
    } // Fusers


    for (var _i2 = fusers.length - 1; _i2 >= 0; _i2--) {
      var _p2 = fusers[_i2];
      _p2.fuse -= dt;

      if (_p2.fuse <= 0) {
        explode(_p2.pos, _p2.color, 60);
        removeAt(fusers, _i2);
      } else {
        roam(_p2.pos, _p2.vel, dts);
      }
    }
  }
  ...
  function draw(ctx) {
    ...
    for (var _i3 = 0, _arr = [lerpers, fusers, roamers]; _i3 < _arr.length; _i3++) {
      var _list = _arr[_i3];

      for (var i = 0; i < _list.length; i++) {
        var p = _list[i];
        ctx.fillStyle = "hsl(".concat(p.color.h, "deg, ").concat(p.color.s, "%, ").concat(p.color.l, "%)"); // TODO: consider precomputing this string for lerpers and fusers
        // ctx.beginPath();
  ...
  var minvec = function minvec(a, b) {
    return [min(a[0], b[0]), min(a[1], b[1])];
  };

  var maxvec = function maxvec(a, b) {
  ...
  var recording = {
    ...
    onmousemove: function onmousemove(e) {
      if (this.mouseDown) {
        var lastUpdate = this.updatedAt;
        this.updatedAt = performance.now();
        var delta = this.updatedAt - lastUpdate;
  ...
  function normalize() {
    ...
    for (var i = 0; i < glyphs.length; i++) {
      var glyph = glyphs[i];
      var origin = glyph.reduce(minvec, [1e9, 1e9]);
      var bound_ = glyph.reduce(maxvec, [0, 0]);
      var bound = setOrigin(origin)(bound_);
      glyphs[i] = glyph.filter(differentFrom(glyph[glyph.length - 1])).filter(takeEvery(2)).map(setOrigin(origin)).map(scaleBy(height / bound[1]));
    }
  }

  var noise = function noise(x) {
    return 2 * x * Math.random() - x;
  };

  function barrage(width, height, color) {
    for (var x = 0; x <= width; x += 30) {
      fusers.push(new Fuser([x, height + 2], [0, -500 * Math.random() - 300], Math.random() * 1000 + 1000, Object.create(color)));
    }
  } // unfinished
  ...
  function sides(width, height, color) {
    ...
    for (var i = 0; i < 3; i++, theta -= Math.PI / 4) {
      fusers.push(new Fuser([0, height], [500 * Math.cos(theta), 500 * Math.sin(theta)], 1500, Object.create(color)));
      fusers.push(new Fuser([width, height], [500 * Math.cos(theta - Math.PI), 500 * Math.sin(theta - Math.PI)], 1500, Object.create(color)));
    }
  }

  function radius(width, height, color) {
    for (var theta = 0; theta > -Math.PI; theta -= Math.PI / 10) {
      for (var abs = 400; abs <= 600; abs += 100) {
        fusers.push(new Fuser([width / 2, height], [abs * Math.cos(theta), abs * Math.sin(theta)], 1500, Object.create(color)));
      }
    }
  }

  var list = [barrage, radius];
  ...
  var getOffset = function getOffset() {
    return new Promise(function (res, rej) {
      ...
      xhr.onload = function () {
        var server = new Date(xhr.response.utc_datetime);
        res(server - new Date());
      };
  ...
  function init() {
    getOffset().then(function (o) {
      return offset = o;
    }).then(function () {
      return console.log('offset found: ', offset);
    }).catch(function () {
      offset = 0;
  ...
  window.addEventListener('resize', function () {
    ...
    world.height = height = document.documentElement.clientHeight || window.innerHeight; // Fun fact! This could possibly execute intertwined with some other drawing function, thus drawing a black particle, for example
  ...
  function print(n) {
    ...
    var translate = function translate(o) {
      return function (p) {
        return [p[0] + o[0], p[1] + o[1]];
      };
    };

    var max_ = function max_(a, b) {
      return a > b ? a : b;
    };

    var maxvec = function maxvec(a, b) {
      return [max_(a[0], b[0]), max_(a[1], b[1])];
    };
    ...
    function push(n) {
      var p = glyphs[n].map(translate([max[0] + 20, 0]));
      points.push.apply(points, _toConsumableArray(p));
    ...
    lerpers.push.apply(lerpers, _toConsumableArray(points.map(translate([width / 2 - max[0] / 2, height / 2 - max[1] / 2])) // translate to center
    .map(function (point) {
      return new Lerper( // [ width / 2 + font.noise(width / 2), height ],
      [width / 2, height], // [ width / 2, height / 2 ],
      point, Object.create(color));
    })));
  }

  var delay = 0; // TODO: review this

  var targetDate = new Date(2020, 0, 1); // why the fuck are months 0-indexed?

  targetDate = new Date(2020, 0, 1, 0, 0, 0);
  var isBeforeTarget = new Date() < targetDate;
  var lastSTo = -1;

  function ny() {
    for (var i = 0; i < 20; i++) {
      console.log("%c Happy New Year!".concat(' '.repeat(i % 2)), 'font-size: 2em; font-family: cursive; font-weight: bold; color: red;');
    }
  }
  ...
  function update$1(delta) {
    ...
    var msTo = targetDate - date();
    var sTo = ~~Math.floor(msTo / 1000);
    ...
    if (!isBeforeTarget || sTo < 1) {
      ...
      if (timer >= delay) {
        ...
        fusers.push(new Fuser([width / 2 + noise(width / 2), height + 2], [noise(100), -400 * Math.random() - 300], Math.random() * 1000 + 1000, randomColor()));
  ...
  function drawFPS(delta) {
    ...
    ctx.fillStyle = 'white';
    ctx.font = '16px Arial';
    ctx.fillText("FPS: ".concat(~~(fps * 100.0) / 100.0), 10, 20);
  }
